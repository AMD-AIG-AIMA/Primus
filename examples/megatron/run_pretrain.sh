...
handle_hipblaslt_tuning() {
    ...
    local TUNE_LOG_PATH=${PRIMUS_PATH}/output/tune_hipblaslt/${MODEL_CONFIG}
...
prepare_dataset() {
    MODEL_CONFIG=$(grep "^[[:space:]]*model:" "$EXP" | awk -F ': ' '{print $2}')
    ...
    if [[ "$MODEL_CONFIG" =~ ^\$\{([^}:]+)(:([^}]+))?\}\.yaml$ ]]; then
        VAR_NAME="${BASH_REMATCH[1]}"
        DEFAULT_VALUE="${BASH_REMATCH[3]}"

        if [[ -n "${!VAR_NAME}" ]]; then
            RESOLVED_MODEL="${!VAR_NAME}.yaml"
        ...
        if [[ "$MODEL_CONFIG" != *.yaml ]]; then
            echo "[ERROR] MODEL must end with .yaml (given: $MODEL_CONFIG)"
            exit 1
        fi
        echo "Using literal MODEL = $RESOLVED_MODEL"
    fi

    MODEL_CONFIG_FILE="$PRIMUS_PATH/primus/configs/models/megatron/${MODEL_CONFIG}"
