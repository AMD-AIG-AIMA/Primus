...
if [ "$NODE_RANK" = "0" ]; then
    ...
    echo "[NODE-$NODE_RANK($HOSTNAME)] NODE_RANK/NNODES: $NODE_RANK/$NNODES"
    echo "[NODE-$NODE_RANK($HOSTNAME)] NCCL_IB_HCA: $NCCL_IB_HCA"
    echo "[NODE-$NODE_RANK($HOSTNAME)] IP_INTERFACE: $IP_INTERFACE"
    echo "[NODE-$NODE_RANK($HOSTNAME)] GPUS_PER_NODE: $GPUS_PER_NODE"
    echo "[NODE-$NODE_RANK($HOSTNAME)] HIP_VISIBLE_DEVICES: $HIP_VISIBLE_DEVICES"
    echo ""

    echo "==========Training experiment info=========="
    echo "[NODE-$NODE_RANK($HOSTNAME)] MODEL_CONFIG: $MODEL_CONFIG"
    echo "[NODE-$NODE_RANK($HOSTNAME)] PRIMUS_PATH: $PRIMUS_PATH"
    echo "[NODE-$NODE_RANK($HOSTNAME)] MEGATRON_PATH: $MEGATRON_PATH"
    echo "[NODE-$NODE_RANK($HOSTNAME)] HF_HOME: $HF_HOME"
    echo "[NODE-$NODE_RANK($HOSTNAME)] TOKENIZED_DATA_PATH: $TOKENIZED_DATA_PATH"
    echo "[NODE-$NODE_RANK($HOSTNAME)] MODEL_CONFIG_FILE: $MODEL_CONFIG_FILE"
    echo "[NODE-$NODE_RANK($HOSTNAME)] TOKENIZER_TYPE: $TOKENIZER_TYPE"
    echo "[NODE-$NODE_RANK($HOSTNAME)] TOKENIZER_MODEL: $TOKENIZER_MODEL"
    echo ""
else
    echo "[NODE-$NODE_RANK($HOSTNAME)] MASTER_ADDR: $MASTER_ADDR"
    echo "[NODE-$NODE_RANK($HOSTNAME)] MASTER_PORT: $MASTER_PORT"
