...
_default_backend: str = "turbo"


def get_backend() -> str:
    global _used_backend, _default_backend
    return _used_backend or _default_backend


def set_backend(backend: str) -> None:
    global _used_backend
    if backend not in ["turbo", "mori"]:
        raise ValueError(f"only support [torbo, mori], unkown backend {backend}")

    if backend == "turbo" and not HAVE_TURBO_BACKEND:
        raise NotImplementedError("DeepEP not support for turbo backend, please install primus_turbo")
    if backend == "mori" and not HAVE_MORI_BACKEND:
...
class FusedDispatch(torch.autograd.Function):
    ...
    def forward(ctx, x, token_indices, token_probs, num_experts, group, previous_event=None):
        ...
        #     print(f"Dispatch Forward: {x.shape}")
        (
            recv_x,
            recv_token_indices,
            recv_token_probs,
    ...
    def backward(ctx, grad_output, grad_token_indices, grad_token_probs, grad_tokens_per_expert, grad_handle):
        ...
        # if torch.distributed.get_rank() == 0:
        #     print(f"Dispatch Backward: {grad_output.shape}")
...
class FusedCombine(torch.autograd.Function):
    ...
    def forward(ctx, x, group, handle, previous_event=None):
        ...
        buffer = get_buffer(group, get_hidden_bytes(x))
        # if torch.distributed.get_rank() == 0:
        #     print(f"Combine Forward: {x.shape}")
    ...
    def backward(ctx, grad_output, previous_event=None):
        ...
        buffer = get_buffer(ctx.group, get_hidden_bytes(grad_output))
        # if torch.distributed.get_rank() == 0:
        #     print(f"Combine Backward: {grad_output.shape}")
