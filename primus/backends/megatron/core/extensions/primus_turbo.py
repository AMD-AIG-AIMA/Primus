...
class PrimusTurboAttention(te.pytorch.DotProductAttention):
    ...
    def forward(
        ...
        backend_type = "ck" if self.cp_param_bundle is None else "triton"
        use_fp8_blockwise = False  # todo add flags
        attn_op = pt.ops.attention_fp8_blockwise if use_fp8_blockwise else pt.ops.attention
        o = attn_op(
            query,
            key,
            value,
