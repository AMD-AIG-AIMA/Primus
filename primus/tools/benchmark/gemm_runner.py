...
def add_gemm_parser(parser: argparse.ArgumentParser):
    ...
    Supports both standalone and CLI subcommand integration.
    """
    # Shape configuration
    ...
    parser.add_argument("--ffn-mult", type=float, default=4.0, help="FFN expansion multiplier")

    parser.add_argument(
        "--op", choices=["mm", "ffn", "proj", "attn-qk", "attn-pv"], default="mm", help="GEMM op type"
    )
...
def resolve_shape(args):
    ...
    if args.op == "ffn":
        return mbs * seq, int(h * ffn_mult), h, False, False
    elif args.op == "proj":
        return mbs * seq, h, int(h * ffn_mult), False, False
    elif args.op == "attn-qk":
        return mbs, seq, h, False, True
    elif args.op == "attn-pv":
        return mbs, h, seq, False, False
    else:
        return mbs * seq, h, h, False, False
...
def run_gemm_benchmark(args):
    ...
    m, n, k, transA, transB = resolve_shape(args)
    dtype = torch.bfloat16 if args.dtype == "bf16" else torch.float16
    result = profile_gemm(m, n, k, dtype, transA, transB)
    ...
    if rank == 0:
        ...
        output_path = os.path.join(args.output, f"gemm_{args.op}_{tag}.md")

        with open(output_path, "w") as f:
            f.write("| Rank | Hostname | M | N | K | Time (s) | TFLOP | TFLOPS | BW (GB/s) |\n")
            f.write("|------|----------|---|---|---|----------|--------|---------|-----------|\n")
            for r, res in enumerate(gathered_results):
                f.write(
                    f"| {r} | {res['hostname']} | {res['m']} | {res['n']} | {res['k']} | "
